...
// Поднимаем WEB-сервер  
   ...
   server.on ( "/button", HTTP_handleButton );
 ...
 */
   ...
   out +=str;
   sprintf(str,"T=%02d H=%d A=%d<br>",Temp,Hum,Avalue);
   out +=str;
   sprintf(str,"Таймер: %02d:%02d</br>",timer/120,(timer/2)%60);
   out +=str;
   out +="Свет: ";
   if( flag_light )out +="включен";
   else out +="выключен";
   out +="</h2>\n";
   out +="<h1><a href='/button'>КНОПКА</a></h1>\n";
   ...
   out+= "\
     ...
     <p><a href=\"/config2\">Настройка контроллера вентилятора</a>\
  </body>\
</html>";
...
"<html>\
  ...
  <body>\
    <h1>Настройка параметров сети</h1>\
     <ul>\
     <li><a href=\"/\">Главная</a>\
     <li><a href=\"/config2\">Настройка контроллера вентилятора</a>\
...
"<html>\
  ...
  <body>\
    <h1>Настройка контроллера вентилятора</h1>\
     <ul>\
     <li><a href=\"/\">Главная</a>\
     <li><a href=\"/config\">Настройка параметров сети</a>\
     <li><a href=\"/reboot\">Перезагрузка</a>\
     <li><a href=\"/default\">Сброс настроек</a>\
     </ul>";
...
"     <form method='get' action='/save'>\
  ...
  out += str;
  out += "'></td></tr>\
         <tr><td>Время работы вентилятора, сек:</td><td><input name='timer' length=32 value='";
...
void HTTP_handleSave(void) {
  ...
  if( server.hasArg("timer")        )EC_Config.TIMER_PERIOD  = atoi(server.arg("timer").c_str());
